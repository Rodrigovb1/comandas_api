...
async def listar_auditoria(
    ...
    recurso: Optional[str] = Query(None, description="Filtrar por recurso (separar múltiplas com vírgula)"),
    data_inicio: Optional[str] = Query(None, description="Data início (YYYY-MM-DD)"),
    data_fim: Optional[str] = Query(None, description="Data fim (YYYY-MM-DD)"),
    skip: int = Query(0, ge=0, description="Número de registros a pular para paginação"),
    limite: int = Query(100, ge=1, le=100, description="Número máximo de registros a retornar"),
    db: Session = Depends(get_db), current_user: FuncionarioAuth = Depends(require_group([1]))
):
    """
    Lista registros de auditoria com filtros opcionais - Apenas adm podem acessar.
    """
    try:
        ...
        if data_inicio:
            try:
                ...
                raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Formato de data_inicio inválido. Use YYYY-MM-DD.")
        ...
        if data_fim:
            try:
                ...
                raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Formato de data_fim inválido. Use YYYY-MM-DD.")

        # Contar total para metadata
        total_count = query.count()

        # Ordenar por data_hora desc, aplicar paginação e limitar
