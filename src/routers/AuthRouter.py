...
async def refresh_token(refresh_data: RefreshTokenRequest, db:Session = Depends(get_db)):
    ...
    try:
        ...
        # ver na ia: pq não funcionario: int = payload.get("id_funcionario")?
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Erro ao renovar token: {str(e)}", headers={"WWW-Authenticate": "Bearer"}, )
...
async def get_current_user_info(current_user: FuncionarioAuth = Depends(get_current_active_user)): # ver pq q a IA sugeriu -> FuncionarioAuth
    """
...
async def logout():
    ...
    Na prática, o logout é implementado no cliente, removendo o token armazenado (ex: localStorage, cookies).
