# Nome do Aluno: Rodrigo Vaisam Bastos
...
async def get_comanda(id: int, request: Request, db: AsyncSession = Depends(get_async_db), current_user: FuncionarioAuth = Depends(get_current_active_user)):
    try:
        ...
        row = result.first()
        # O que é um row?É uma tupla que contém os objetos ComandaDB, FuncionarioDB e ClienteDB resultantes do select.
        # E o que é uma tupla? É uma estrutura de dados imutável que pode conter múltiplos valores. No caso do SQLAlchemy, quando fazemos um select com joins, o resultado
        # é uma tupla onde cada elemento corresponde a uma das entidades selecionadas. Por exemplo, se selecionamos ComandaDB, FuncionarioDB e ClienteDB, a tupla terá
        # três elementos: o primeiro será a instância de ComandaDB, o segundo será a instância de FuncionarioDB e o terceiro será a instância de ClienteDB.
        ...
        # O que difere isso do row = result.first()? É que aqui estamos desempacotando a tupla resultante do select em três variáveis distintas: comanda, funcionario e cliente.
...
async def get_comandas(
    ...
    data_inicio: Optional[str] = Query(None, description="Filtrar por data inicial"),
    data_fim: Optional[str] = Query(None, description="Filtrar por data final"),
