...
# Scheme para extair token do header Authorization: Bearer <token>
security = HTTPBearer()

# Dependency para validar token e retornar usuário final
def get_current_user(
    ...
    if cpf is None or id_funcionario is None:
        raise HTTPException(
            ...
            detail="Token inválido - dados incompletos", headers={"WWW-Authenticate": "Bearer"},
        )
    ...
    if not funcionario:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="Usuário não encontrado", headers={"WWW-Authenticate": "Bearer"},
        )
    ...
    if funcionario.cpf != cpf:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="Token inválido - CPF não corresponde", headers={"WWW-Authenticate": "Bearer"},
        )
...
# Dependency para verificar se o usuário está ativo (pode ser estendido para incluir status, bloqueio, etc.)
def get_current_active_user(current_user: FuncionarioAuth = Depends(get_current_user)) -> FuncionarioAuth:
    """Dependency que verifica se o usuário está ativo (pode ser estendido para incluir status)"""
    # Aqui você pode adicionar lógica para verificar se o usuário está ativo, bloqueado, etc.
    # Por exemplo, verificar um campo 'ativo' no banco de dados
    return current_user

# Dependency para verificar se o usuário é tem um grupo específico (ex: admin)
